...
export default class ClassesController {
  ...
  async create(request: Request, response: Response) {
    ...
    try {
      ...
      const classSchendule = schedule.map((schenduleItem: PropsSchenduleItem) => {
        return {
          class_id,
      ...
      return response.status(201).send('Dados cadastrados com sucesso!')
    } catch (err) {
      trx.rollback()
